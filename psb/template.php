...
function psb_js_alter(&$javascript) {
	// bootstrap requires a later version of jquery
	$javascript['misc/jquery.js']['data'] = drupal_get_path('theme', 'psb') . '/js/jquery-1.11.3.min.js';
}
...
function psb_preprocess_page(&$vars, $hook) {
	...
	if ($vars['main_menu'] && module_exists('menu_block')) {
		$vars['primary_nav'] = __psb_mega_menu();
...
function psb_menu_block_tree_alter(&$tree, &$config) {
...
function psb_block_view_alter(&$data, $block) {
...
function psb_form_search_api_page_search_form_default_search_alter(&$form, &$form_state, $form_id) {
	// update the placeholder text for the search form in the header
	$form['keys_1']['#attributes']['placeholder'] = 'Search website';
}

function psb_theme($existing, $type, $theme, $path) {
...
function psb_menu_link(array $variables) {
...
function psb_file_link($variables) {
	$file = $variables['file'];
